...
public class OutGroupEvaluateActivity extends TitleActivity implements OnClickListener{
	...
	private String startoutgroupevaluate = "startoutgroupevaluate.do";
	private String endoutgroupevaluate = "endoutgroupevaluate.do";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		starturl = baseInfo.getUrl()+startoutgroupevaluate;
		endurl = baseInfo.getUrl()+endoutgroupevaluate;
	...
	public void start(){
		...
		if (time.length() != 0) {
			startoutgroupevaluate(time);
		}else {
			Toast.makeText(OutGroupEvaluateActivity.this, "请输入评价时间",1).show();;
		}

	}
	//开始评价的时候调用该方法，客户端向服务器发送请求，设置开始评价标签，学生端才能进行评价。
	public void startoutgroupevaluate(String time) {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				timer = new CountDownTimer(millisInFuture*1000, 1000) {
					...
					public void onFinish() {
						...
						endtoutgroupevaluate(time);
					}

					private void endtoutgroupevaluate(String time) {
						// TODO Auto-generated method stub
						RequestParams params = new RequestParams();
						params.addQueryStringParameter("seId",Integer.toString(seId));
						http.send(HttpRequest.HttpMethod.GET,
